...
import { tenantApp, tenantParty, tenantBots, tenantWhatsApp } from '@/config/tenant.config'

const CITIZEN_BOT_USERNAME = tenantBots.citizen.username
const TELEGRAM_URL = `https://t.me/${CITIZEN_BOT_USERNAME}`
const WA_ME_NUMBER =
  (import.meta.env.VITE_WHATSAPP_WAME_NUMBER as string | undefined)?.replace(/\D/g, '') ||
  tenantWhatsApp.waMeNumber.replace(/\D/g, '')
const WHATSAPP_ENABLED =
  tenantWhatsApp.enabled || !!import.meta.env.VITE_WHATSAPP_WAME_NUMBER
const WHATSAPP_URL = WA_ME_NUMBER
  ? `https://wa.me/${WA_ME_NUMBER}?text=${encodeURIComponent(tenantWhatsApp.prefillMessage || 'Hi')}`
  : null
          ...
          <div className="lp-citizen-cards">
            ...
            {WHATSAPP_ENABLED && WHATSAPP_URL ? (
              <div className="lp-card lp-card-hover lp-card-centered">
                ...
                <div>
                  <div className="lp-card-label">Chat on WhatsApp</div>
                  <p className="lp-card-body-text">
                    Report issues via <strong>WhatsApp</strong> — text, photos, or voice. Same intake flow as Telegram.
                    {tenantWhatsApp.displayNumber ? (
                      <> Number: <strong>{tenantWhatsApp.displayNumber}</strong></>
                    ) : null}
                  </p>
                  <a
                    href={WHATSAPP_URL}
